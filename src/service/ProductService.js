...
const API_URL = "https://localhost:7208/api/Product";
const ProductService = {
  ...
  updateProduct: async (product) => {
    const response = await axios.put(
      `${API_URL}/UpdateForManagement?id=${product.productID}`,
      product,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  getProductsBase: async (params) => {
    ...
    const response = await axios.get(
      `${API_URL}/base/getProducts${queryString}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
  ...
  getTopProductInMonth: async (top) => {
    const response = await axios.get(
      `${API_URL}/base/GetTopProductInMonth?top=${top}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
